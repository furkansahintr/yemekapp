...
function _dacInjectStyles() {
  ...
  // ── P7.3 Modal shell + onay + OTP + başarı + askıya alma ──
  parts.push(
    '/* Modal backdrop + shell */',
    '.dac-modal-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(5px);display:flex;align-items:center;justify-content:center;z-index:96;opacity:0;transition:opacity .24s ease;padding:16px}',
    '.dac-modal-backdrop.open{opacity:1}',
    '.dac-modal{width:100%;max-width:400px;background:linear-gradient(180deg,#FDF4FF 0%,#fff 50%);border-radius:20px;overflow:hidden;transform:scale(.92);transition:transform .26s cubic-bezier(.2,.9,.25,1);max-height:92vh;overflow-y:auto}',
    '.dac-modal-backdrop.open .dac-modal{transform:scale(1)}',
    '/* Confirm modal */',
    '.dac-confirm{padding:24px 20px 18px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px}',
    '.dac-confirm-emoji{font-size:54px;line-height:1;animation:dacWave 1.6s ease-in-out infinite}',
    '@keyframes dacWave{0%,100%{transform:rotate(0deg)}25%{transform:rotate(-15deg)}75%{transform:rotate(15deg)}}',
    '.dac-confirm-title{font-size:18px;font-weight:800;color:#BE185D}',
    '.dac-confirm-body{font-size:12.5px;color:var(--text-primary);line-height:1.6;max-width:360px}',
    '.dac-confirm-body b{font-weight:800;color:#7C3AED}',
    '.dac-confirm-dates{display:flex;align-items:center;gap:10px;padding:12px;background:#F5F3FF;border-radius:12px;width:100%;justify-content:center;margin-top:4px}',
    '.dac-dr{display:flex;flex-direction:column;align-items:center;gap:2px;min-width:90px}',
    '.dac-dr span{font-size:10px;color:var(--text-muted);font-weight:600;letter-spacing:.3px;text-transform:uppercase}',
    '.dac-dr b{font-size:12px;font-weight:800;color:#7C3AED}',
    '.dac-dr-arrow{display:flex;align-items:center;justify-content:center}',
    '.dac-confirm-hint{display:flex;align-items:flex-start;gap:6px;padding:10px 11px;background:rgba(139,92,246,.06);border-radius:10px;font-size:11px;color:var(--text-primary);line-height:1.5;width:100%}',
    '.dac-confirm-hint b{font-weight:700;color:#7C3AED}',
    '.dac-confirm-btns{display:flex;gap:8px;width:100%;margin-top:6px}',
    '.dac-confirm-btns > button{flex:1}',
    '/* OTP modal */',
    '.dac-otp{position:relative;padding:24px 20px 18px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px}',
    '.dac-otp-close{position:absolute;top:10px;right:10px;width:30px;height:30px;display:flex;align-items:center;justify-content:center;cursor:pointer}',
    '.dac-otp-close:active{transform:scale(.92)}',
    '.dac-otp-ico{width:66px;height:66px;border-radius:50%;background:rgba(139,92,246,.12);display:flex;align-items:center;justify-content:center}',
    '.dac-otp-title{font-size:17px;font-weight:800;color:var(--text-primary)}',
    '.dac-otp-body{font-size:12px;color:var(--text-muted);line-height:1.5;max-width:320px}',
    '.dac-otp-body b{font-weight:700;color:var(--text-primary)}',
    '.dac-otp-input-wrap{width:100%;padding:4px 0}',
    '.dac-otp-input{width:100%;max-width:260px;padding:14px;border:1.5px solid rgba(139,92,246,.3);background:#fff;color:var(--text-primary);border-radius:12px;font-size:22px;font-weight:800;text-align:center;letter-spacing:8px;outline:none;font-variant-numeric:tabular-nums;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}',
    '.dac-otp-input:focus{border-color:#8B5CF6}',
    '.dac-otp-resend{padding:6px 12px;border:none;background:transparent;color:#8B5CF6;font-size:11.5px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:4px}',
    '.dac-otp-hint{display:inline-flex;align-items:center;gap:5px;font-size:10.5px;color:var(--text-muted);font-weight:500}',
    '.dac-otp .dac-confirm-btns{margin-top:8px}',
    '.dac-btn-danger.disabled{opacity:.4;cursor:not-allowed;transform:none;box-shadow:none}',
    '/* Success modal */',
    '.dac-success{padding:26px 20px 22px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px}',
    '.dac-success-ico{animation:dacHeartPop .6s ease}',
    '@keyframes dacHeartPop{0%{transform:scale(0)}50%{transform:scale(1.25)}100%{transform:scale(1)}}',
    '.dac-success-title{font-size:19px;font-weight:800;color:#BE185D;letter-spacing:.2px}',
    '.dac-success-body{font-size:12.5px;color:var(--text-primary);line-height:1.55;max-width:340px}',
    '.dac-success-body b{font-weight:800;color:#7C3AED}',
    '.dac-success-tip{display:inline-flex;align-items:center;gap:5px;padding:8px 12px;background:linear-gradient(135deg,#FEF3C7,#FDE68A);border-radius:999px;font-size:11px;color:#92400E;font-weight:700;margin-top:2px}',
    '/* Suspended view */',
    '.dac-susp-card{padding:28px 20px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:10px;background:linear-gradient(180deg,rgba(139,92,246,.05),rgba(236,72,153,.04));border:1px dashed rgba(139,92,246,.22);border-radius:18px}',
    '.dac-susp-title{font-size:18px;font-weight:800;color:var(--text-primary);margin-top:4px}',
    '.dac-susp-sub{font-size:12px;color:var(--text-muted)}',
    '.dac-susp-sub b{font-weight:800;color:#EF4444}',
    '.dac-susp-countdown{display:flex;flex-direction:column;align-items:center;padding:16px 24px;background:#fff;border:2px solid #EDE9FE;border-radius:16px;margin:4px 0}',
    '.dac-countdown-num{font-size:52px;font-weight:800;color:#7C3AED;line-height:1;font-variant-numeric:tabular-nums;text-shadow:0 2px 4px rgba(124,58,237,.15)}',
    '.dac-countdown-lbl{font-size:10px;font-weight:700;color:var(--text-muted);letter-spacing:2px;margin-top:4px}',
    '.dac-susp-date{font-size:11.5px;color:var(--text-muted)}',
    '.dac-susp-date b{font-weight:800;color:var(--text-primary)}',
    '.dac-susp-hint{font-size:10.5px;color:var(--text-muted);margin-top:4px;font-style:italic;max-width:280px;line-height:1.45}'
  );
